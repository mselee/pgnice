// Modified version of https://github.com/4lDO2/ioprio-rs/
...
impl Priority {
    ...
    pub fn class(self) -> Class {
        ...
        match class_raw {
            1 => Class::Realtime(RtPriorityLevel::from_level(data as u8).unwrap()),
            2 => Class::BestEffort(BePriorityLevel::from_level(data as u8).unwrap()),
            3 => Class::Idle,
            _ => Class::None,
        }
    }
...
fn target_which_who(target: Target) -> [nix::libc::c_int; 2] {
    match target {
        Target::Process(pid) => [1, pid.as_raw() as nix::libc::c_int],
        Target::ProcessGroup(pgid) => [2, pgid.as_raw() as nix::libc::c_int],
        Target::User(uid) => [3, uid.as_raw() as nix::libc::c_int],
    }
}
...
pub fn get_priority(target: Target) -> nix::Result<Priority> {
    ...
    let res = unsafe { nix::libc::syscall(nix::libc::SYS_ioprio_get, which, who) };
...
pub fn set_priority(target: Target, priority: Priority) -> nix::Result<()> {
    ...
    let res = unsafe {
        nix::libc::syscall(
            ...
            which,
            who,
            priority.inner as nix::libc::c_int,
        )
    };
